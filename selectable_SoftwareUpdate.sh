...
if [[ ! -z "${noReboots[@]}" ]]; then
    ...
    --icon-file "$swuIcon" --icon-height 80 --icon-width 80 --width 600 --posY top )
...
if [[ "$osVers" -lt 15 ]]; then
    readSWUs=$( cat /tmp/SWULIST | awk -F"," '/[rR]ecommended/{print $2,$1}' | sed -e 's/[0-9]*K \[recommended\][ *]//g;s/\[restart\] */◀ /g' | sed 's/[   ]//g' )
    progSWUs=$( cat /tmp/SWULIST | awk -F"," '/[rR]ecommended/{print $2,$1}' | sed -e 's/[0-9]*K \[recommended\][ *]//g;s/\[restart\] *//g' | sed 's/[ ]//g' )
else
    readSWUs=$( cat /tmp/SWULIST | awk -F"," '/[rR]ecommended/{print $2,$1,$5}' | sed -e 's/Recommended//g;s/Action\: restart */ ◀ /g;s/Title\: //g;s/Version:\ //g;')
    progSWUs=$( cat /tmp/SWULIST | awk -F"," '/[rR]ecommended/{print $2,$1,$5}' | sed -e 's/Recommended//g;s/Action\: restart *//g;s/Title\: //g;s/Version:\ //g;')
fi
installSWUs=$( cat /tmp/SWULIST | grep -v -i 'recommended' | awk -F'\\* ' '/\*/{print $NF}' )
